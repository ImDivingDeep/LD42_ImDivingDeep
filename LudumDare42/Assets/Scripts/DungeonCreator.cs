...
public class DungeonCreator : MonoBehaviour {
    ...
    void SetupTilesArray()
    {
        ...
        tileScale = floorTiles[0].transform.localScale;
        ...
        for (int i = 0; i < rooms.Length; i++)
        {
            ...
            for (int j = 0; j < currentRoom.roomWidth; j++)
            {
                ...
                for (int k = 0; k < currentRoom.roomHeight; k++)
                {
                    ...
                    if (yCoord + 1 < tiles[xCoord].Length)
                        if(k == currentRoom.roomHeight - 1 && tiles[xCoord][yCoord + 1] != TileType.Floor)
                            tiles[xCoord][yCoord + 1] = TileType.UpperWall;

                    if(yCoord - 1 > 0)
                        if (k == 0 && tiles[xCoord][yCoord - 1] != TileType.Floor)
                            tiles[xCoord][yCoord - 1] = TileType.WallDown;
                }
            }
        }
    }
    ...
    void SetTilesValuesForCorridors()
    {
        ...
        for (int i = 0; i < corridors.Length; i++)
        {
            ...
            for (int j = 0; j < currentCorridor.corridorLength; j++)
            {
                ...
                if (yCoord + 1 < tiles[xCoord].Length)
                    if (currentCorridor.direction == Direction.East || currentCorridor.direction == Direction.West)
                        if(tiles[xCoord][yCoord + 1] != TileType.Floor)
                            tiles[xCoord][yCoord + 1] = TileType.UpperWall;
    ...
    void InstantiateTiles()
    {
        ...
        for (int i = 0; i < tiles.Length; i++)
        {
            for (int j = 0; j < tiles[i].Length; j++)
            {
                ...
                if (tiles[i][j] == TileType.UpperWall)
                {
                    // ... instantiate a wall over the top.
                    InstantiateFromArray(wallupTiles, i, j);
                }
                else if (tiles[i][j] == TileType.WallDown)
                {
                    InstantiateFromArray(walldownTiles, i, j);
                }
    ...
    void InstantiateFromArray(GameObject[] prefabs, float xCoord, float yCoord)
    {
        ...
        Vector3 position = new Vector3(xCoord * tileScale.x, yCoord * tileScale.y, 0f);
